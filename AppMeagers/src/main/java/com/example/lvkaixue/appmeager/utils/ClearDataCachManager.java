 ...
 * 清楚整个应用缓存
 */
public class ClearDataCachManager {
    ...
    public static boolean clearAllCache(Context context) {
        ...
        boolean ISCLEAREXTERCACHE=false;
        int ISCANEXTER = 0;
        ISCLEARCACHE = deleteDir(context.getCacheDir());
        if (Environment.getExternalStorageState().equals(
                Environment.MEDIA_MOUNTED)) {
            ISCANEXTER = 1;
            ISCLEAREXTERCACHE = deleteDir(context.getExternalCacheDir());
        }
        if(ISCLEARCACHE && ISCANEXTER==0){
            return true;
        }else if(ISCLEARCACHE && ISCANEXTER == 1 && ISCLEAREXTERCACHE ){
            return true;
        }else {
            return false;
        }
